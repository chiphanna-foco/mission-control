...
cmd_sweep() {
    ...
    if ! curl -s -f "$MISSION_CONTROL_URL/api/health" > /dev/null 2>&1; then
    ...
    if echo "$response" | jq empty 2>/dev/null; then
        ...
        if [ "$status" = "success" ] || [ "$status" = "partial" ]; then
            print_status "success" "Sweep completed with status: $status"
            ...
            print_status "error" "Sweep failed with status: $status"
